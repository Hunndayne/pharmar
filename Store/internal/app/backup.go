...
func (s *Service) createFileServiceToken(ctx context.Context) (string, error) {
	...
	if method == nil {
		return "", fmt.Errorf("unsupported JWT algorithm: %s", s.cfg.JWTAlgorithm)
	}
...
func (s *Service) syncBackupToR2KeepLatest(ctx context.Context, record domain.BackupRecord) error {
	...
	go func() {
		...
		if err := writer.WriteField("category", "backup"); err != nil {
			pipeWriter.CloseWithError(err)
			writeErrCh <- err
			return
		}
		if err := writer.WriteField("ref_type", "store_backup"); err != nil {
			pipeWriter.CloseWithError(err)
			writeErrCh <- err
			return
		}
		if err := writer.WriteField("ref_id", record.ID); err != nil {
			pipeWriter.CloseWithError(err)
			writeErrCh <- err
			return
		}

		filePart, err := writer.CreateFormFile("file", record.Filename)
		if err != nil {
			pipeWriter.CloseWithError(err)
			writeErrCh <- err
			return
	...
	uploadURL := fmt.Sprintf("%s/api/v1/file/upload", strings.TrimRight(fileServiceURL, "/"))
	...
	if err != nil {
		_ = pipeReader.Close()
		return fmt.Errorf("upload backup to file-service: %w", err)
	}
	defer uploadResp.Body.Close()

	if writeErr := <-writeErrCh; writeErr != nil {
		return fmt.Errorf("prepare multipart backup upload: %w", writeErr)
	}
	...
	if uploadResp.StatusCode != http.StatusCreated && uploadResp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("file-service upload failed (%d): %s", uploadResp.StatusCode, string(body))
	}
	...
	for _, f := range listed.Files {
		...
		deleteURL := fmt.Sprintf("%s/api/v1/file/%s", strings.TrimRight(fileServiceURL, "/"), fileID)
		deleteReq, err := http.NewRequestWithContext(ctx, http.MethodDelete, deleteURL, nil)
		if err != nil {
			log.Printf("auto-backup R2 cleanup skip file=%s: %v", fileID, err)
			continue
		}
		...
		if err != nil {
			log.Printf("auto-backup R2 cleanup delete file=%s failed: %v", fileID, err)
			continue
		}
		_ = deleteResp.Body.Close()
		if deleteResp.StatusCode != http.StatusOK {
			log.Printf("auto-backup R2 cleanup delete file=%s returned status=%d", fileID, deleteResp.StatusCode)
		}
	}
...
func (s *Service) checkAndRunAutoBackup(ctx context.Context, lastBackup *time.Time) {
	...
	record, err := s.CreateBackup(ctx, "Sao luu tu dong", "system")
	if err != nil {
