    ...
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-3xl border border-white/60 bg-white/70">
        <div className="space-y-3 p-3 md:hidden">
          {loading ? <p className="rounded-2xl bg-white px-4 py-3 text-sm text-ink-600">{TEXT.loading}</p> : null}
          {!loading && rows.length === 0 ? (
            <p className="rounded-2xl bg-white px-4 py-3 text-sm text-ink-600">{TEXT.noData}</p>
          ) : null}
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold text-ink-900">{supplierName}</p>
                        <p className="text-xs text-ink-600">{item.code}</p>
                      </div>
                      <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${item.is_active ? 'bg-brand-500/15 text-brand-600 border border-brand-500/30' : 'bg-ink-600/10 text-ink-600 border border-ink-600/20'}`}>
                    ...
                    {expandedId === item.id ? (
                      <div className="mt-3 rounded-xl border border-ink-900/10 bg-fog-50 p-3 text-xs text-ink-700">
                        ...
                        <p><span className="font-semibold text-ink-900">MÃ£ sá»‘ thuáº¿:</span> {supplierTaxCode}</p>
                        <p><span className="font-semibold text-ink-900">Äá»‹a chá»‰:</span> {supplierAddress}</p>
                        <p><span className="font-semibold text-ink-900">Ghi chÃº:</span> {supplierNote}</p>
                      </div>
                    ) : null}
                  </article>
                )
